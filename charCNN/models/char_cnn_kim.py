...
class CharCNNKim(object):
    ...
    def _build_model(self):
        ...
        # Embedding layers
        x = Embedding(self.alphabet_size + 4, self.embedding_size, input_length=self.input_size)(inputs)
        # Convolution layers
        convolution_output = []
    ...
    def train(self, data_type,training_inputs, training_labels,
        ...
        # Start training
        print("Training CharCNNKim model with best vocab_size ")
        ...
        if data_type=="ILI":
            self.model.save("models/ILI/charCNNKim%s_%s%s"%(epochs,model_type,self.alphabet_size))
        if data_type=="gdi":
            self.model.save("models/gdi/charCNNKim%s_%s%s"%(epochs,model_type,self.alphabet_size))
        if data_type=="aoc":
            self.model.save("models/aoc/charCNNKim%s_%s%s"%(epochs,model_type,self.alphabet_size))
        if data_type=="adi":
            self.model.save("models/adi/charCNNKim%s_%s%s"%(epochs,model_type,self.alphabet_size))
        if data_type=="nadi":
            self.model.save("models/nadi/charCNNKim%s_%s%s"%(epochs,model_type,self.alphabet_size))
        #self.model.evaluate(validation_inputs, validation_labels, batch_size=config["training"]["batch_size"], verbose=1)
